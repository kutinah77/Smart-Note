            ...
            Button(
                onClick = {
                    coroutineScope.launch {
                        val success = viewModel.deleteMultipleHabayebCustomersSuspended(selectedCustomerIds)
                        if (success) {
            ...
            Button(
                onClick = {
                    if (editedNameStr.isNotBlank()) {
                        coroutineScope.launch {
                            val success = viewModel.updateHabayebCustomerNameSuspended(customer.id, editedNameStr.trim())
                            if (success) {
                                com.example.ui.helper.VibrationHelper.vibrateSuccess(context)
