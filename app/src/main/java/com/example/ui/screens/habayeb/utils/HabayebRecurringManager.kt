...
object HabayebRecurringManager {
    ...
    private fun saveConfigsList(context: Context, configs: List<RecurringConfig>) {
        ...
        prefs.edit().putString(KEY_CONFIGS, array.toString()).commit()
    }

    suspend fun checkAndExecuteRecurring(context: Context, viewModel: FinanceViewModel, onExecuted: (Int) -> Unit = {}) {
            ...
            for (config in configs) {
                ...
                if (matchesTimestamps.isNotEmpty()) {
                    ...
                    for (ts in finalToExecute) {
                        viewModel.addHabayebTransactionSuspended(
